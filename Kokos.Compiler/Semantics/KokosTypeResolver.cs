...
public sealed class KokosTypeResolver : IKokosVisitor<KokosType>
{
    ...
    private List<KokosStructField> ResolveFields(IReadOnlyList<KokosFieldNode> fieldNodes)
    {
        ...
        for (var i = 0; i < fieldNodes.Count; i++)
        {
            ...
            var isReadOnly = KokosModifierMapper.IsReadOnlyOf(fieldNode.Type, _table);

            fields.Add(new KokosStructField(fieldNode.Name, fieldNode.IndexToken is not null, i, type, ownership, isReadOnly));
        }
    ...
    public KokosType VisitLiteralNull(KokosLiteralNullNode node) => throw NotAType(nameof(KokosLiteralNullNode));
    public KokosType VisitNullForgiving(KokosNullForgivingNode node) => throw NotAType(nameof(KokosNullForgivingNode));
    ...
    public KokosType VisitArrayLiteral(KokosArrayLiteralNode node) => throw NotAType(nameof(KokosArrayLiteralNode));
    public KokosType VisitTupleConstruction(KokosTupleConstructionNode node) => throw NotAType(nameof(KokosTupleConstructionNode));
